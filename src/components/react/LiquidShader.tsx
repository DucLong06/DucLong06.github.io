...
export default function LiquidShader({ className }: Props) {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      // Bail if the component unmounted while ogl was loading — avoids attaching
      // listeners / starting the RAF loop with no way to stop them.
      if (cancelled) return;
    ...
    init().then((fn) => {
      // If we unmounted before init resolved, run its cleanup immediately.
      if (cancelled) fn?.();
      else cleanupFn = fn;
    });
